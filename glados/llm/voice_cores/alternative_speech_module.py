...
class AlternativeSpeechModule:
    def __init__(self, tts: Kokoro,
        ...
        Args:
            tts: The TTS synthesizer instance. Kokoro(tts_model_path, voices_path)
            ...
            speaking_lock: threading.yyEvent, locked while speaking.
        """
        self._tts = tts
        ...
        while not self._stop_event.is_set():
            try:
                ...
                if generated_text == "<EOS>":  # End-of-stream signal
                    logger.info("Received end-of-stream signal, clearing lock")
                    self._speaking_lock.clear()
                    self.event_system.publish(EventMessage("system", "listen_for_response", {}))  # todo replace this with something faster
                    continue
                ...
                logger.info("Locking speaking thread")
    ...
    def _say(self, text: str):
        ...
        try:
            ...
            samples, sample_rate = self._tts.create(text, voice="bf_isabella", speed=1.0, lang="en-us")
            samples = self.resample_audio(samples, sample_rate, SAMPLE_RATE)
    ...
    def play_audio(self, audio):
        ...
        try:
            ...
            logger.info(f"Playing {len(audio)} samples at {SAMPLE_RATE} Hz, duration: {duration:.2f} seconds.")
...
#                     self.event_system.publish(EventMessage("system", "enable_stt", {}))
#                     self.stt_enabled = True
#                     logger.info("TTS complete. Re-enabling STT.")
#
#     def _say(self, text: str):
#         """
#         Generate and play TTS audio from text (synchronous implementation).
#         """
#         try:
#             logger.info(f"Generating TTS for: {text}")
#
...
#                 self.stt_enabled = False
#
#             # Generate TTS output using the Kokoro TTS instance
#             samples, sample_rate = self._tts.create(
#                 text, voice="bf_isabella", speed=1.0, lang="en-us"
#             )
#
#             # Log the TTS generation details
#             logger.debug(f"TTS Output: {len(samples)} samples at {sample_rate} Hz")
#
#             # Resample the audio if the sample rate is not as expected
#             samples = self.resample_audio(samples, sample_rate, SAMPLE_RATE)
#
#             # Play the generated audio
#             self.play_audio(samples)
